...
const jwtaccess = require("../src/auth/jwtaccess");
mocha_1.describe('jwt', () => {
    ...
    mocha_1.it('should create a dummy refresh token string', () => {
    ...
    mocha_1.it('should get an initial access token', done => {
    ...
    mocha_1.it('should accept scope as string', done => {
        const jwt = new src_1.JWT({
    ...
    mocha_1.it('can get obtain new access token when scopes are set', done => {
        const jwt = new src_1.JWT({
            email: 'foo@serviceaccount.com',
            keyFile: PEM_PATH,
    ...
    mocha_1.it('should emit an event for tokens', done => {
    ...
    mocha_1.it('can obtain new access token when scopes are set', async () => {
        const jwt = new src_1.JWT({
            email: 'foo@serviceaccount.com',
            keyFile: PEM_PATH,
            scopes: ['http://bar', 'http://foo'],
    ...
    mocha_1.it('gets a jwt header access token', async () => {
    ...
    mocha_1.it('gets a jwt header access token with key id', async () => {
        const keys = keypair(512 /* bitsize of private key */);
        const jwt = new src_1.JWT({
            email: 'foo@serviceaccount.com',
            key: keys.private,
    ...
    mocha_1.it('should accept additionalClaims', async () => {
        const keys = keypair(512 /* bitsize of private key */);
    ...
    mocha_1.it('should accept additionalClaims that include a target_audience', async () => {
        const keys = keypair(512 /* bitsize of private key */);
        const jwt = new src_1.JWT({
            email: 'foo@serviceaccount.com',
            key: keys.private,
    ...
    mocha_1.it('should refresh token if expired', done => {
        const jwt = new src_1.JWT({
            email: 'foo@serviceaccount.com',
            keyFile: PEM_PATH,
            scopes: ['http://bar', 'http://foo'],
    ...
    mocha_1.it('should refresh if access token will expired soon and time to refresh before expiration is set', done => {
        const jwt = new src_1.JWT({
            email: 'foo@serviceaccount.com',
            keyFile: PEM_PATH,
            scopes: ['http://bar', 'http://foo'],
    ...
    mocha_1.it('should not refresh if access token will not expire soon and time to refresh before expiration is set', done => {
    ...
    mocha_1.it('should refresh token if the server returns 403', done => {
    ...
    mocha_1.it('should return expiry_date in milliseconds', async () => {
        const jwt = new src_1.JWT({
            email: 'foo@serviceaccount.com',
            keyFile: PEM_PATH,
            scopes: ['http://bar', 'http://foo'],
    ...
    mocha_1.it('createScoped should clone stuff', () => {
        const jwt = new src_1.JWT({
            email: 'foo@serviceaccount.com',
            keyFile: '/path/to/key.pem',
            keyId: '101',
    ...
    mocha_1.it('createScoped should handle string scope', () => {
        const jwt = new src_1.JWT({
            email: 'foo@serviceaccount.com',
            keyFile: '/path/to/key.pem',
            scopes: ['http://bar', 'http://foo'],
    ...
    mocha_1.it('createScoped should handle array scope', () => {
        const jwt = new src_1.JWT({
            email: 'foo@serviceaccount.com',
            keyFile: '/path/to/key.pem',
            scopes: ['http://bar', 'http://foo'],
    ...
    mocha_1.it('createScoped should handle null scope', () => {
        const jwt = new src_1.JWT({
            email: 'foo@serviceaccount.com',
            keyFile: '/path/to/key.pem',
            scopes: ['http://bar', 'http://foo'],
    ...
    mocha_1.it('createScoped should set scope when scope was null', () => {
        const jwt = new src_1.JWT({
            email: 'foo@serviceaccount.com',
            keyFile: '/path/to/key.pem',
            subject: 'bar@subjectaccount.com',
    ...
    mocha_1.it('createScoped should handle nulls', () => {
        const jwt = new src_1.JWT();
        const clone = jwt.createScoped('hi');
    ...
    mocha_1.it('createScoped should not return the original instance', () => {
        const jwt = new src_1.JWT({
            email: 'foo@serviceaccount.com',
            keyFile: '/path/to/key.pem',
            scopes: ['http://bar', 'http://foo'],
    ...
    mocha_1.it('should error on missing client_id', () => {
    ...
    mocha_1.it('should error on missing client_secret', () => {
        const json = createRefreshJSON();
    ...
    mocha_1.it('should error on missing refresh_token', () => {
        const json = createRefreshJSON();
    ...
    mocha_1.it('fromStream should read the stream and create a jwt', done => {
    ...
    mocha_1.it('fromAPIKey should error without api key', () => {
        assert.throws(() => {
            // Test verifies invalid parameter tests, which requires cast to any.
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    mocha_1.it('fromAPIKey should error with invalid api key type', () => {
        const KEY = 'test';
        assert.throws(() => {
            // Test verifies invalid parameter tests, which requires cast to any.
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    mocha_1.it('fromAPIKey should set the .apiKey property on the instance', () => {
        const KEY = 'test';
        jwt.fromAPIKey(KEY);
        assert.strictEqual(jwt.apiKey, KEY);
    });
    mocha_1.it('getCredentials should handle a key', async () => {
    ...
    mocha_1.it('getCredentials should handle a p12 keyFile', async () => {
    ...
    mocha_1.it('getCredentials should handle a json keyFile', async () => {
        const jwt = new src_1.JWT();
        jwt.fromJSON(json);
        const { private_key, client_email } = await jwt.getCredentials();
    ...
    mocha_1.it('getRequestHeaders populates x-goog-user-project for JWT client', async () => {
        ...
        assert(client instanceof src_1.JWT);
        // If a URL isn't provided to authorize, the OAuth2Client super class is
        // executed, which was already exercised.
        const headers = await client.getRequestHeaders('http:/example.com/my_test_service');
        assert.strictEqual(headers['x-goog-user-project'], 'fake-quota-project');
    });
    mocha_1.it('should return an ID token for fetchIdToken', async () => {
        const jwt = new src_1.JWT({
            email: 'foo@serviceaccount.com',
            key: PEM_CONTENTS,
            subject: 'ignored@subjectaccount.com',
    ...
    mocha_1.it('should throw an error if ID token is not set', async () => {
        const jwt = new src_1.JWT({
            email: 'foo@serviceaccount.com',
            key: PEM_CONTENTS,
            subject: 'ignored@subjectaccount.com',
    ...
    mocha_1.describe('self-signed JWT', () => {
        mocha_1.afterEach(() => {
            sandbox.restore();
        });
        mocha_1.it('uses self signed JWT when no scopes are provided', async () => {
        ...
        mocha_1.it('uses self signed JWT when default scopes are provided', async () => {
        ...
        mocha_1.it('does not use self signed JWT if target_audience provided', async () => {
            const JWTAccess = sandbox.stub(jwtaccess, 'JWTAccess').returns({
                getRequestHeaders: sinon.stub().returns({}),
            });
            const keys = keypair(512 /* bitsize of private key */);
        ...
        mocha_1.it('returns headers from cache, prior to their expiry time', async () => {
        ...
        mocha_1.it('creates a new self-signed JWT, if headers are close to expiring', async () => {
            const sign = sandbox.stub(jws, 'sign').returns('abc123');
            const getExpirationTime = sandbox
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                .stub(jwtaccess.JWTAccess, 'getExpirationTime')
        ...
        mocha_1.it('returns no headers when no scopes or audiences are provided', async () => {
            const jwt = new src_1.JWT({
                email: 'foo@serviceaccount.com',
                key: fs.readFileSync(PEM_PATH, 'utf8'),
                scopes: [],
